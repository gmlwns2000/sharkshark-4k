...
class FNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        out = F.interpolate(
            self.decoder1(out), scale_factor=2.0, mode='bilinear', align_corners=False)
        out = F.interpolate(
            self.decoder2(out), scale_factor=2.0, mode='bilinear', align_corners=False)
        out = F.interpolate(
            self.decoder3(out), scale_factor=2.0, mode='bilinear', align_corners=False)
...
class FRNet(BaseSequenceGenerator):
    ...
    def forward(self, lr_curr, lr_prev, hr_prev):
        ...
        # compute hr_curr
        n, c, in_h, in_w = hr_prev_warp.size()
        out_h, out_w = in_h // self.scale, in_w // self.scale

        x_reshaped = hr_prev_warp.reshape(n, c, out_h, self.scale, out_w, self.scale)
        x_reshaped = x_reshaped.permute(0, 3, 5, 1, 2, 4)
        hr_prev_warp_reshaped = x_reshaped.reshape(n, self.scale * self.scale * c, out_h, out_w)

        hr_curr = self.srnet(lr_curr, hr_prev_warp_reshaped)
